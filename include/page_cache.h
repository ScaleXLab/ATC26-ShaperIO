...
//        printf("Cache Extra Reads: %llu\n", v);
    ...
    page_cache_t(const uint64_t ps, const uint64_t np, const uint32_t cudaDevice, const Controller& ctrl, const uint64_t max_range, const std::vector<Controller*>& ctrls) {
        ...
        if (ps <= this->pages_dma.get()->page_size) {
            ...
            std::cout << "DMA contiguous: " << this->pages_dma.get()->contiguous
                      << ", dma_page_size: " << this->pages_dma.get()->page_size
                      << ", n_ioaddrs: " << this->pages_dma.get()->n_ioaddrs << std::endl;
            for (size_t i = 0; i < std::min((size_t)4, (size_t)np); i++) {
                std::cout << "  page[" << i << "] PRP1=0x" << std::hex << temp1[i]
                          << " PRP2=0x" << temp2[i]
                          << " diff=" << std::dec << (int64_t)(temp2[i] - temp1[i]) << std::endl;
            }
