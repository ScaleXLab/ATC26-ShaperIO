...
/*
 * Query GPU physical addresses (BAR addresses) for a device buffer that was
 * previously mapped with nvm_dma_map_device. These are the raw GPU physical
 * addresses from nvidia_p2p_page_table_t, suitable for use as PFNs in GPU
 * page table entries.
 *
 * phys_addrs must point to an array of at least n_pages uint64_t entries.
 * n_pages must match the number of GPU pages in the mapping (size / 64KB).
 */
int nvm_dma_query_phys_addrs(const nvm_ctrl_t* ctrl, void* devptr,
                             size_t n_pages, uint64_t* phys_addrs);
