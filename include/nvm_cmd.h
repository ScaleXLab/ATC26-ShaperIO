...
enum nvm_io_command_set
{
    ...
    NVM_IO_DATASET_MGMT     = NVM_CMD_OPCODE(0, 2, 1)   // 09h
};

#define NVM_DSM_ATTR_DEALLOCATE 0x1u
...
enum nvm_admin_command_set
{
    ...
    NVM_ADMIN_FORMAT_NVM    = NVM_CMD_OPCODE(1, 0, 0)   // 80h
};
...
/*
 * Set command's dataset management fields (DWORD10-11).
 * nr is zero-based (nr = number_of_ranges - 1).
 */
__device__ __host__ static inline
void nvm_cmd_dsm(nvm_cmd_t* cmd, uint32_t nr, uint32_t attr)
{
    cmd->dword[10] = nr;
    cmd->dword[11] = attr;
}
