...
static void admin_format_nvm(nvm_cmd_t* cmd, uint32_t ns_id,
                             uint8_t ses, uint8_t lbaf, uint8_t mset,
                             uint8_t pi, uint8_t pil)
{
    nvm_cmd_header(cmd, 0, NVM_ADMIN_FORMAT_NVM, ns_id);
    nvm_cmd_data_ptr(cmd, 0, 0);
...
int nvm_admin_format(nvm_aq_ref ref, uint32_t ns_id,
                     uint8_t ses, uint8_t lbaf, uint8_t mset,
                     uint8_t pi, uint8_t pil)
{
    nvm_cmd_t command;
    ...
    admin_format_nvm(&command, ns_id, ses, lbaf, mset, pi, pil);

    int err = nvm_raw_rpc(ref, &command, &completion);
    if (!nvm_ok(err))
    {
        dprintf("Format NVM failed: %s\n", nvm_strerror(err));
        return err;
    }
