...
{
    if (ctrl == NULL || devptr == NULL || phys_addrs == NULL || n_pages == 0)
    {
        return EINVAL;
    }
    ...
    struct device* dev = (struct device*) c->device;

    struct nvm_ioctl_query_phys request = {
        .vaddr_start = (uint64_t)(uintptr_t) devptr,
        .n_pages = n_pages,
        .phys_addrs = phys_addrs
    };

    int err = ioctl(dev->fd, NVM_QUERY_DEVICE_PHYS_ADDRS, &request);

    _nvm_ctrl_put(c);

    if (err < 0)
    {
        dprintf("Query phys addrs ioctl failed: %s\n", strerror(errno));
        return errno;
    }
