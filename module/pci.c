...
#ifdef _CUDA
        case NVM_QUERY_DEVICE_PHYS_ADDRS:
        {
            struct nvm_ioctl_query_phys qreq;
            struct map* qmap;
            struct gpu_region* gd;
            uint64_t* phys_buf;
            unsigned long i;

            if (copy_from_user(&qreq, (void __user*) arg, sizeof(qreq)))
                return -EFAULT;

            qmap = map_find(&device_list, qreq.vaddr_start);
            if (qmap == NULL)
                return -EINVAL;

            gd = (struct gpu_region*) qmap->data;
            if (gd == NULL || gd->pages == NULL)
                return -EINVAL;

            if (qreq.n_pages != gd->pages->entries)
                return -EINVAL;

            phys_buf = kmalloc_array(qreq.n_pages, sizeof(uint64_t), GFP_KERNEL);
            if (!phys_buf)
                return -ENOMEM;

            for (i = 0; i < qreq.n_pages; i++)
                phys_buf[i] = gd->pages->pages[i]->physical_address;

            if (copy_to_user((void __user*) qreq.phys_addrs, phys_buf,
                             qreq.n_pages * sizeof(uint64_t))) {
                kfree(phys_buf);
                return -EFAULT;
            }
            kfree(phys_buf);
